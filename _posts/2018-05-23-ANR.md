...
我们知道出现问题java bt会在data/anr 保存相关的bt 信息，所以data/anr 里面的数据针对分析问题是很关键的，正常执行adb pull data/anr 我们都可以获取一些比较完整的trace信息。下面是一小段system server 的
java trace的开始，从第一行开始我们来按每一行解析其所代表的含义。
